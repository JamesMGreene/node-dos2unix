...
function processAllFiles(fileList, options) {
  /*jshint validthis:true */
  var processor = this;
  ...
  var taskList = fileList.map(function(filePath) {
    return function(callback) {
      // `processFile` does not `reject` any promises, so we can skip the `.fail` chain here (for now, at least)
      Q.when(processFileFn(filePath), function(result) {
        callback(null, result);
      });
    };
  });
  var joiningCallback = function(err, results) {
    // There should never be an `err` here
    if (err) {
      deferred.reject(err);
      return;
    }
  ...
  async.parallelLimit(taskList, options.maxConcurrency, joiningCallback);

  // Catch up and move on...
  return Q.when(deferred.promise, function(statuses) {
...
Dos2UnixConverter.prototype.process = function(globPatterns, options) {
  ...
  validateGlobPatternsP(globPatterns)
    ...
    .then(function(fileList) {
      return processAllFilesP(fileList, opts);
    })
    .then(onDoneP)
    .fail(onFailP)
    .done();
